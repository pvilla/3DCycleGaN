...
defaultkwargs = {
    ...
    'num_iters': 1000000,                           # total number of training iterations
    'num_samples': 1000000,                           # number of randomly sampled image patches
    'batch_size': 4,                                
    ...
    'pretrained': False,                            # path to pretrained networks - else None
    'HPoptimizer': False,                           # Do you want to use our dynamic hyperparameter updater? - It sucks but is better than manually choosing HPs :)
    'kickstart': False,                             # replaces generated images with input images during the first 150 steps. Prevents color inversions - only use if the intensities of both domains is similar
...
def train(**kwargs):
    ...
    while model.step < model.num_iters:
        ...
        #for iteration in range(model.num_iters):
            ...
            if model.step % model.print_loss_freq_iter == model.print_loss_freq_iter - 1:
                ...
                model.print_current_losses(losses)
...
# uncomment to train the 2x network for enhancement of the 1ms, 800nm dataset
    train(run_name = '2x_1ms_4',
        data_A = 'dataset/T700_GF_0p8um_1ms.json',
